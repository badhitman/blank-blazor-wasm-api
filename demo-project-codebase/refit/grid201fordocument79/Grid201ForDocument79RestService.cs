...
{
	/// <inheritdoc/>
	public class Grid201ForDocument79RestService : IGrid201ForDocument79RestService
	{
		private readonly IGrid201ForDocument79RefitService _service;
		private readonly ILogger<Grid201ForDocument79RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid201ForDocument79RestService(IGrid201ForDocument79RefitService set_service, ILogger<Grid201ForDocument79RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid201ForDocument79_ResponseModel> FirstAsync(int id)
		{
			Grid201ForDocument79_ResponseModel result = new();
			try
			{
				ApiResponse<Grid201ForDocument79_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid201ForDocument79_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid201ForDocument79_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid201ForDocument79_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid201ForDocument79_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid201ForDocument79_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
