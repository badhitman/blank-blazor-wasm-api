...
{
	/// <inheritdoc/>
	public class Grid98ForDocument42RestService : IGrid98ForDocument42RestService
	{
		private readonly IGrid98ForDocument42RefitService _service;
		private readonly ILogger<Grid98ForDocument42RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid98ForDocument42RestService(IGrid98ForDocument42RefitService set_service, ILogger<Grid98ForDocument42RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid98ForDocument42_ResponseModel> FirstAsync(int id)
		{
			Grid98ForDocument42_ResponseModel result = new();
			try
			{
				ApiResponse<Grid98ForDocument42_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid98ForDocument42_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid98ForDocument42_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid98ForDocument42_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid98ForDocument42_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid98ForDocument42_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
