...
{
	/// <inheritdoc/>
	public class Grid57ForDocument24RestService : IGrid57ForDocument24RestService
	{
		private readonly IGrid57ForDocument24RefitService _service;
		private readonly ILogger<Grid57ForDocument24RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid57ForDocument24RestService(IGrid57ForDocument24RefitService set_service, ILogger<Grid57ForDocument24RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid57ForDocument24_ResponseModel> FirstAsync(int id)
		{
			Grid57ForDocument24_ResponseModel result = new();
			try
			{
				ApiResponse<Grid57ForDocument24_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid57ForDocument24_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid57ForDocument24_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid57ForDocument24_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid57ForDocument24_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid57ForDocument24_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
