...
{
	/// <inheritdoc/>
	public class Grid79ForDocument34RestService : IGrid79ForDocument34RestService
	{
		private readonly IGrid79ForDocument34RefitService _service;
		private readonly ILogger<Grid79ForDocument34RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid79ForDocument34RestService(IGrid79ForDocument34RefitService set_service, ILogger<Grid79ForDocument34RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid79ForDocument34_ResponseModel> FirstAsync(int id)
		{
			Grid79ForDocument34_ResponseModel result = new();
			try
			{
				ApiResponse<Grid79ForDocument34_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid79ForDocument34_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid79ForDocument34_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid79ForDocument34_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid79ForDocument34_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid79ForDocument34_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
