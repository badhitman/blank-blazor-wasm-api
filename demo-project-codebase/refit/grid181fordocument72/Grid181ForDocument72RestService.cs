...
{
	/// <inheritdoc/>
	public class Grid181ForDocument72RestService : IGrid181ForDocument72RestService
	{
		private readonly IGrid181ForDocument72RefitService _service;
		private readonly ILogger<Grid181ForDocument72RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid181ForDocument72RestService(IGrid181ForDocument72RefitService set_service, ILogger<Grid181ForDocument72RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid181ForDocument72_ResponseModel> FirstAsync(int id)
		{
			Grid181ForDocument72_ResponseModel result = new();
			try
			{
				ApiResponse<Grid181ForDocument72_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid181ForDocument72_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid181ForDocument72_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid181ForDocument72_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid181ForDocument72_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid181ForDocument72_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
