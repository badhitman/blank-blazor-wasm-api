...
{
	/// <inheritdoc/>
	public class Document25_ModelRestService : IDocument25_ModelRestService
	{
		private readonly IDocument25_ModelRefitService _service;
		private readonly ILogger<Document25_ModelRestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Document25_ModelRestService(IDocument25_ModelRefitService set_service, ILogger<Document25_ModelRestService> set_logger)
		{
		...
		{
			Document25_Model_ResponseModel result = new();
			try
			{
				ApiResponse<Document25_Model_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Document25_Model_ResponseListModel result = new();
			try
			{
				ApiResponse<Document25_Model_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Document25_Model_ResponsePaginationModel> SelectAsync(PaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Document25_Model_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
