...
{
	/// <inheritdoc/>
	public class Grid212ForDocument84RestService : IGrid212ForDocument84RestService
	{
		private readonly IGrid212ForDocument84RefitService _service;
		private readonly ILogger<Grid212ForDocument84RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid212ForDocument84RestService(IGrid212ForDocument84RefitService set_service, ILogger<Grid212ForDocument84RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid212ForDocument84_ResponseModel> FirstAsync(int id)
		{
			Grid212ForDocument84_ResponseModel result = new();
			try
			{
				ApiResponse<Grid212ForDocument84_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid212ForDocument84_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid212ForDocument84_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid212ForDocument84_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid212ForDocument84_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid212ForDocument84_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
