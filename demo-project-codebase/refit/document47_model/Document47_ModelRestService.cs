...
{
	/// <inheritdoc/>
	public class Document47_ModelRestService : IDocument47_ModelRestService
	{
		private readonly IDocument47_ModelRefitService _service;
		private readonly ILogger<Document47_ModelRestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Document47_ModelRestService(IDocument47_ModelRefitService set_service, ILogger<Document47_ModelRestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Document47_Model_ResponseModel> FirstAsync(int id)
		{
			Document47_Model_ResponseModel result = new();
			try
			{
				ApiResponse<Document47_Model_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Document47_Model_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Document47_Model_ResponseListModel result = new();
			try
			{
				ApiResponse<Document47_Model_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Document47_Model_ResponsePaginationModel> SelectAsync(PaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Document47_Model_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
