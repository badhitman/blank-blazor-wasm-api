...
{
	/// <inheritdoc/>
	public class Grid184ForDocument73RestService : IGrid184ForDocument73RestService
	{
		private readonly IGrid184ForDocument73RefitService _service;
		private readonly ILogger<Grid184ForDocument73RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid184ForDocument73RestService(IGrid184ForDocument73RefitService set_service, ILogger<Grid184ForDocument73RestService> set_logger)
		{
		...
		{
			Grid184ForDocument73_ResponseModel result = new();
			try
			{
				ApiResponse<Grid184ForDocument73_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid184ForDocument73_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid184ForDocument73_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid184ForDocument73_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid184ForDocument73_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
