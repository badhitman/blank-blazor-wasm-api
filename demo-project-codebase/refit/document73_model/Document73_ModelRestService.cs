...
{
	/// <inheritdoc/>
	public class Document73_ModelRestService : IDocument73_ModelRestService
	{
		private readonly IDocument73_ModelRefitService _service;
		private readonly ILogger<Document73_ModelRestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Document73_ModelRestService(IDocument73_ModelRefitService set_service, ILogger<Document73_ModelRestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Document73_Model_ResponseModel> FirstAsync(int id)
		{
			Document73_Model_ResponseModel result = new();
			try
			{
				ApiResponse<Document73_Model_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Document73_Model_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Document73_Model_ResponseListModel result = new();
			try
			{
				ApiResponse<Document73_Model_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Document73_Model_ResponsePaginationModel> SelectAsync(PaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Document73_Model_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
