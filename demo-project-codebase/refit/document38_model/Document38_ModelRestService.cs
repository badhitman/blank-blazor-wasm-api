...
{
	/// <inheritdoc/>
	public class Document38_ModelRestService : IDocument38_ModelRestService
	{
		private readonly IDocument38_ModelRefitService _service;
		private readonly ILogger<Document38_ModelRestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Document38_ModelRestService(IDocument38_ModelRefitService set_service, ILogger<Document38_ModelRestService> set_logger)
		{
		...
		{
			Document38_Model_ResponseModel result = new();
			try
			{
				ApiResponse<Document38_Model_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Document38_Model_ResponseListModel result = new();
			try
			{
				ApiResponse<Document38_Model_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Document38_Model_ResponsePaginationModel> SelectAsync(PaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Document38_Model_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
