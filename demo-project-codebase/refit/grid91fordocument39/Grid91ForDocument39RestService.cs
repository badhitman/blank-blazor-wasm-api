...
{
	/// <inheritdoc/>
	public class Grid91ForDocument39RestService : IGrid91ForDocument39RestService
	{
		private readonly IGrid91ForDocument39RefitService _service;
		private readonly ILogger<Grid91ForDocument39RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid91ForDocument39RestService(IGrid91ForDocument39RefitService set_service, ILogger<Grid91ForDocument39RestService> set_logger)
		{
		...
		{
			Grid91ForDocument39_ResponseModel result = new();
			try
			{
				ApiResponse<Grid91ForDocument39_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid91ForDocument39_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid91ForDocument39_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid91ForDocument39_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid91ForDocument39_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
