...
{
	/// <inheritdoc/>
	public class Grid220ForDocument87RestService : IGrid220ForDocument87RestService
	{
		private readonly IGrid220ForDocument87RefitService _service;
		private readonly ILogger<Grid220ForDocument87RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid220ForDocument87RestService(IGrid220ForDocument87RefitService set_service, ILogger<Grid220ForDocument87RestService> set_logger)
		{
		...
		{
			Grid220ForDocument87_ResponseModel result = new();
			try
			{
				ApiResponse<Grid220ForDocument87_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid220ForDocument87_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid220ForDocument87_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid220ForDocument87_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid220ForDocument87_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
