...
{
	/// <inheritdoc/>
	public class Grid179ForDocument71RestService : IGrid179ForDocument71RestService
	{
		private readonly IGrid179ForDocument71RefitService _service;
		private readonly ILogger<Grid179ForDocument71RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid179ForDocument71RestService(IGrid179ForDocument71RefitService set_service, ILogger<Grid179ForDocument71RestService> set_logger)
		{
		...
		{
			Grid179ForDocument71_ResponseModel result = new();
			try
			{
				ApiResponse<Grid179ForDocument71_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid179ForDocument71_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid179ForDocument71_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid179ForDocument71_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid179ForDocument71_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
