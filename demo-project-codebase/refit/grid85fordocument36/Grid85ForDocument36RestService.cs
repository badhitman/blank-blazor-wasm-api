...
{
	/// <inheritdoc/>
	public class Grid85ForDocument36RestService : IGrid85ForDocument36RestService
	{
		private readonly IGrid85ForDocument36RefitService _service;
		private readonly ILogger<Grid85ForDocument36RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid85ForDocument36RestService(IGrid85ForDocument36RefitService set_service, ILogger<Grid85ForDocument36RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid85ForDocument36_ResponseModel> FirstAsync(int id)
		{
			Grid85ForDocument36_ResponseModel result = new();
			try
			{
				ApiResponse<Grid85ForDocument36_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid85ForDocument36_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid85ForDocument36_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid85ForDocument36_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid85ForDocument36_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid85ForDocument36_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
