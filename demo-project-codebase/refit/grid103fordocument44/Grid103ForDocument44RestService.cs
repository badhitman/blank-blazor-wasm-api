...
{
	/// <inheritdoc/>
	public class Grid103ForDocument44RestService : IGrid103ForDocument44RestService
	{
		private readonly IGrid103ForDocument44RefitService _service;
		private readonly ILogger<Grid103ForDocument44RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid103ForDocument44RestService(IGrid103ForDocument44RefitService set_service, ILogger<Grid103ForDocument44RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid103ForDocument44_ResponseModel> FirstAsync(int id)
		{
			Grid103ForDocument44_ResponseModel result = new();
			try
			{
				ApiResponse<Grid103ForDocument44_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid103ForDocument44_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid103ForDocument44_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid103ForDocument44_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid103ForDocument44_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid103ForDocument44_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
