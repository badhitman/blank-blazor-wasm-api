...
{
	/// <inheritdoc/>
	public class Document76_ModelRestService : IDocument76_ModelRestService
	{
		private readonly IDocument76_ModelRefitService _service;
		private readonly ILogger<Document76_ModelRestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Document76_ModelRestService(IDocument76_ModelRefitService set_service, ILogger<Document76_ModelRestService> set_logger)
		{
		...
		{
			Document76_Model_ResponseModel result = new();
			try
			{
				ApiResponse<Document76_Model_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Document76_Model_ResponseListModel result = new();
			try
			{
				ApiResponse<Document76_Model_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Document76_Model_ResponsePaginationModel> SelectAsync(PaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Document76_Model_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
