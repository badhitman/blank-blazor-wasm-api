...
{
	/// <inheritdoc/>
	public class Grid218ForDocument86RestService : IGrid218ForDocument86RestService
	{
		private readonly IGrid218ForDocument86RefitService _service;
		private readonly ILogger<Grid218ForDocument86RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid218ForDocument86RestService(IGrid218ForDocument86RefitService set_service, ILogger<Grid218ForDocument86RestService> set_logger)
		{
		...
		{
			Grid218ForDocument86_ResponseModel result = new();
			try
			{
				ApiResponse<Grid218ForDocument86_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid218ForDocument86_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid218ForDocument86_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid218ForDocument86_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid218ForDocument86_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
