...
{
	/// <inheritdoc/>
	public class Grid59ForDocument25RestService : IGrid59ForDocument25RestService
	{
		private readonly IGrid59ForDocument25RefitService _service;
		private readonly ILogger<Grid59ForDocument25RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid59ForDocument25RestService(IGrid59ForDocument25RefitService set_service, ILogger<Grid59ForDocument25RestService> set_logger)
		{
		...
		{
			Grid59ForDocument25_ResponseModel result = new();
			try
			{
				ApiResponse<Grid59ForDocument25_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid59ForDocument25_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid59ForDocument25_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid59ForDocument25_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid59ForDocument25_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
