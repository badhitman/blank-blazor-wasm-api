...
{
	/// <inheritdoc/>
	public class Grid112ForDocument47RestService : IGrid112ForDocument47RestService
	{
		private readonly IGrid112ForDocument47RefitService _service;
		private readonly ILogger<Grid112ForDocument47RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid112ForDocument47RestService(IGrid112ForDocument47RefitService set_service, ILogger<Grid112ForDocument47RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid112ForDocument47_ResponseModel> FirstAsync(int id)
		{
			Grid112ForDocument47_ResponseModel result = new();
			try
			{
				ApiResponse<Grid112ForDocument47_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid112ForDocument47_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid112ForDocument47_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid112ForDocument47_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid112ForDocument47_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid112ForDocument47_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
