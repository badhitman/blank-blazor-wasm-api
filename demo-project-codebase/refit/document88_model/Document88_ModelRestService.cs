...
{
	/// <inheritdoc/>
	public class Document88_ModelRestService : IDocument88_ModelRestService
	{
		private readonly IDocument88_ModelRefitService _service;
		private readonly ILogger<Document88_ModelRestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Document88_ModelRestService(IDocument88_ModelRefitService set_service, ILogger<Document88_ModelRestService> set_logger)
		{
		...
		{
			Document88_Model_ResponseModel result = new();
			try
			{
				ApiResponse<Document88_Model_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Document88_Model_ResponseListModel result = new();
			try
			{
				ApiResponse<Document88_Model_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Document88_Model_ResponsePaginationModel> SelectAsync(PaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Document88_Model_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
