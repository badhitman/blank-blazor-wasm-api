...
{
	/// <inheritdoc/>
	public class Grid104ForDocument44RestService : IGrid104ForDocument44RestService
	{
		private readonly IGrid104ForDocument44RefitService _service;
		private readonly ILogger<Grid104ForDocument44RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid104ForDocument44RestService(IGrid104ForDocument44RefitService set_service, ILogger<Grid104ForDocument44RestService> set_logger)
		{
		...
		{
			Grid104ForDocument44_ResponseModel result = new();
			try
			{
				ApiResponse<Grid104ForDocument44_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid104ForDocument44_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid104ForDocument44_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid104ForDocument44_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid104ForDocument44_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
