...
{
	/// <inheritdoc/>
	public class Grid190ForDocument75RestService : IGrid190ForDocument75RestService
	{
		private readonly IGrid190ForDocument75RefitService _service;
		private readonly ILogger<Grid190ForDocument75RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid190ForDocument75RestService(IGrid190ForDocument75RefitService set_service, ILogger<Grid190ForDocument75RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid190ForDocument75_ResponseModel> FirstAsync(int id)
		{
			Grid190ForDocument75_ResponseModel result = new();
			try
			{
				ApiResponse<Grid190ForDocument75_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid190ForDocument75_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid190ForDocument75_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid190ForDocument75_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid190ForDocument75_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid190ForDocument75_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
