...
{
	/// <inheritdoc/>
	public class Grid115ForDocument49RestService : IGrid115ForDocument49RestService
	{
		private readonly IGrid115ForDocument49RefitService _service;
		private readonly ILogger<Grid115ForDocument49RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid115ForDocument49RestService(IGrid115ForDocument49RefitService set_service, ILogger<Grid115ForDocument49RestService> set_logger)
		{
		...
		{
			Grid115ForDocument49_ResponseModel result = new();
			try
			{
				ApiResponse<Grid115ForDocument49_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid115ForDocument49_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid115ForDocument49_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid115ForDocument49_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid115ForDocument49_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
