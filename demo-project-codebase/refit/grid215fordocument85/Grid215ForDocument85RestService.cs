...
{
	/// <inheritdoc/>
	public class Grid215ForDocument85RestService : IGrid215ForDocument85RestService
	{
		private readonly IGrid215ForDocument85RefitService _service;
		private readonly ILogger<Grid215ForDocument85RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid215ForDocument85RestService(IGrid215ForDocument85RefitService set_service, ILogger<Grid215ForDocument85RestService> set_logger)
		{
		...
		{
			Grid215ForDocument85_ResponseModel result = new();
			try
			{
				ApiResponse<Grid215ForDocument85_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid215ForDocument85_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid215ForDocument85_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid215ForDocument85_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid215ForDocument85_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
