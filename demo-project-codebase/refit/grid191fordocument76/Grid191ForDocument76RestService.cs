...
{
	/// <inheritdoc/>
	public class Grid191ForDocument76RestService : IGrid191ForDocument76RestService
	{
		private readonly IGrid191ForDocument76RefitService _service;
		private readonly ILogger<Grid191ForDocument76RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid191ForDocument76RestService(IGrid191ForDocument76RefitService set_service, ILogger<Grid191ForDocument76RestService> set_logger)
		{
		...
		{
			Grid191ForDocument76_ResponseModel result = new();
			try
			{
				ApiResponse<Grid191ForDocument76_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid191ForDocument76_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid191ForDocument76_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid191ForDocument76_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid191ForDocument76_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
