...
{
	/// <inheritdoc/>
	public class Grid221ForDocument87RestService : IGrid221ForDocument87RestService
	{
		private readonly IGrid221ForDocument87RefitService _service;
		private readonly ILogger<Grid221ForDocument87RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid221ForDocument87RestService(IGrid221ForDocument87RefitService set_service, ILogger<Grid221ForDocument87RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid221ForDocument87_ResponseModel> FirstAsync(int id)
		{
			Grid221ForDocument87_ResponseModel result = new();
			try
			{
				ApiResponse<Grid221ForDocument87_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid221ForDocument87_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid221ForDocument87_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid221ForDocument87_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid221ForDocument87_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid221ForDocument87_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
