...
{
	/// <inheritdoc/>
	public class Grid182ForDocument72RestService : IGrid182ForDocument72RestService
	{
		private readonly IGrid182ForDocument72RefitService _service;
		private readonly ILogger<Grid182ForDocument72RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid182ForDocument72RestService(IGrid182ForDocument72RefitService set_service, ILogger<Grid182ForDocument72RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid182ForDocument72_ResponseModel> FirstAsync(int id)
		{
			Grid182ForDocument72_ResponseModel result = new();
			try
			{
				ApiResponse<Grid182ForDocument72_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid182ForDocument72_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid182ForDocument72_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid182ForDocument72_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid182ForDocument72_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid182ForDocument72_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
