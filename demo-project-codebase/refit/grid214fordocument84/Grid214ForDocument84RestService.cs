...
{
	/// <inheritdoc/>
	public class Grid214ForDocument84RestService : IGrid214ForDocument84RestService
	{
		private readonly IGrid214ForDocument84RefitService _service;
		private readonly ILogger<Grid214ForDocument84RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid214ForDocument84RestService(IGrid214ForDocument84RefitService set_service, ILogger<Grid214ForDocument84RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid214ForDocument84_ResponseModel> FirstAsync(int id)
		{
			Grid214ForDocument84_ResponseModel result = new();
			try
			{
				ApiResponse<Grid214ForDocument84_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid214ForDocument84_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid214ForDocument84_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid214ForDocument84_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid214ForDocument84_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid214ForDocument84_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
