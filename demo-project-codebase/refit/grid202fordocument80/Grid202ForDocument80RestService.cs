...
{
	/// <inheritdoc/>
	public class Grid202ForDocument80RestService : IGrid202ForDocument80RestService
	{
		private readonly IGrid202ForDocument80RefitService _service;
		private readonly ILogger<Grid202ForDocument80RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid202ForDocument80RestService(IGrid202ForDocument80RefitService set_service, ILogger<Grid202ForDocument80RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid202ForDocument80_ResponseModel> FirstAsync(int id)
		{
			Grid202ForDocument80_ResponseModel result = new();
			try
			{
				ApiResponse<Grid202ForDocument80_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid202ForDocument80_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid202ForDocument80_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid202ForDocument80_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid202ForDocument80_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid202ForDocument80_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
