...
{
	/// <inheritdoc/>
	public class Grid185ForDocument73RestService : IGrid185ForDocument73RestService
	{
		private readonly IGrid185ForDocument73RefitService _service;
		private readonly ILogger<Grid185ForDocument73RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid185ForDocument73RestService(IGrid185ForDocument73RefitService set_service, ILogger<Grid185ForDocument73RestService> set_logger)
		{
		...
		{
			Grid185ForDocument73_ResponseModel result = new();
			try
			{
				ApiResponse<Grid185ForDocument73_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid185ForDocument73_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid185ForDocument73_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid185ForDocument73_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid185ForDocument73_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
