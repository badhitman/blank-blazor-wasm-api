...
{
	/// <inheritdoc/>
	public class Grid75ForDocument33RestService : IGrid75ForDocument33RestService
	{
		private readonly IGrid75ForDocument33RefitService _service;
		private readonly ILogger<Grid75ForDocument33RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid75ForDocument33RestService(IGrid75ForDocument33RefitService set_service, ILogger<Grid75ForDocument33RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid75ForDocument33_ResponseModel> FirstAsync(int id)
		{
			Grid75ForDocument33_ResponseModel result = new();
			try
			{
				ApiResponse<Grid75ForDocument33_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid75ForDocument33_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid75ForDocument33_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid75ForDocument33_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid75ForDocument33_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid75ForDocument33_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
