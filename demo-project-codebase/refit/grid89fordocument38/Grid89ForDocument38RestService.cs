...
{
	/// <inheritdoc/>
	public class Grid89ForDocument38RestService : IGrid89ForDocument38RestService
	{
		private readonly IGrid89ForDocument38RefitService _service;
		private readonly ILogger<Grid89ForDocument38RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid89ForDocument38RestService(IGrid89ForDocument38RefitService set_service, ILogger<Grid89ForDocument38RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid89ForDocument38_ResponseModel> FirstAsync(int id)
		{
			Grid89ForDocument38_ResponseModel result = new();
			try
			{
				ApiResponse<Grid89ForDocument38_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid89ForDocument38_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid89ForDocument38_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid89ForDocument38_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid89ForDocument38_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid89ForDocument38_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
