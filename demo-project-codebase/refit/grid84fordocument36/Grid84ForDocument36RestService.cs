...
{
	/// <inheritdoc/>
	public class Grid84ForDocument36RestService : IGrid84ForDocument36RestService
	{
		private readonly IGrid84ForDocument36RefitService _service;
		private readonly ILogger<Grid84ForDocument36RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid84ForDocument36RestService(IGrid84ForDocument36RefitService set_service, ILogger<Grid84ForDocument36RestService> set_logger)
		{
		...
		{
			Grid84ForDocument36_ResponseModel result = new();
			try
			{
				ApiResponse<Grid84ForDocument36_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid84ForDocument36_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid84ForDocument36_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid84ForDocument36_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid84ForDocument36_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
