...
{
	/// <inheritdoc/>
	public class Grid50ForDocument21RestService : IGrid50ForDocument21RestService
	{
		private readonly IGrid50ForDocument21RefitService _service;
		private readonly ILogger<Grid50ForDocument21RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid50ForDocument21RestService(IGrid50ForDocument21RefitService set_service, ILogger<Grid50ForDocument21RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid50ForDocument21_ResponseModel> FirstAsync(int id)
		{
			Grid50ForDocument21_ResponseModel result = new();
			try
			{
				ApiResponse<Grid50ForDocument21_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid50ForDocument21_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid50ForDocument21_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid50ForDocument21_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid50ForDocument21_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid50ForDocument21_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
