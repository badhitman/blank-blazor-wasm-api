...
{
	/// <inheritdoc/>
	public class Grid116ForDocument49RestService : IGrid116ForDocument49RestService
	{
		private readonly IGrid116ForDocument49RefitService _service;
		private readonly ILogger<Grid116ForDocument49RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid116ForDocument49RestService(IGrid116ForDocument49RefitService set_service, ILogger<Grid116ForDocument49RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid116ForDocument49_ResponseModel> FirstAsync(int id)
		{
			Grid116ForDocument49_ResponseModel result = new();
			try
			{
				ApiResponse<Grid116ForDocument49_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid116ForDocument49_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid116ForDocument49_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid116ForDocument49_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid116ForDocument49_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid116ForDocument49_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
