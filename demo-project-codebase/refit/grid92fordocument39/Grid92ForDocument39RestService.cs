...
{
	/// <inheritdoc/>
	public class Grid92ForDocument39RestService : IGrid92ForDocument39RestService
	{
		private readonly IGrid92ForDocument39RefitService _service;
		private readonly ILogger<Grid92ForDocument39RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid92ForDocument39RestService(IGrid92ForDocument39RefitService set_service, ILogger<Grid92ForDocument39RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid92ForDocument39_ResponseModel> FirstAsync(int id)
		{
			Grid92ForDocument39_ResponseModel result = new();
			try
			{
				ApiResponse<Grid92ForDocument39_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid92ForDocument39_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid92ForDocument39_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid92ForDocument39_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid92ForDocument39_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid92ForDocument39_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
