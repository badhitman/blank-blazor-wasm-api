...
{
	/// <inheritdoc/>
	public class Grid49ForDocument21RestService : IGrid49ForDocument21RestService
	{
		private readonly IGrid49ForDocument21RefitService _service;
		private readonly ILogger<Grid49ForDocument21RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid49ForDocument21RestService(IGrid49ForDocument21RefitService set_service, ILogger<Grid49ForDocument21RestService> set_logger)
		{
		...
		{
			Grid49ForDocument21_ResponseModel result = new();
			try
			{
				ApiResponse<Grid49ForDocument21_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid49ForDocument21_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid49ForDocument21_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid49ForDocument21_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid49ForDocument21_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
