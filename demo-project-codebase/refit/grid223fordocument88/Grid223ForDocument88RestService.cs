...
{
	/// <inheritdoc/>
	public class Grid223ForDocument88RestService : IGrid223ForDocument88RestService
	{
		private readonly IGrid223ForDocument88RefitService _service;
		private readonly ILogger<Grid223ForDocument88RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid223ForDocument88RestService(IGrid223ForDocument88RefitService set_service, ILogger<Grid223ForDocument88RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid223ForDocument88_ResponseModel> FirstAsync(int id)
		{
			Grid223ForDocument88_ResponseModel result = new();
			try
			{
				ApiResponse<Grid223ForDocument88_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid223ForDocument88_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid223ForDocument88_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid223ForDocument88_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid223ForDocument88_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid223ForDocument88_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
