...
{
	/// <inheritdoc/>
	public class Document85_ModelRestService : IDocument85_ModelRestService
	{
		private readonly IDocument85_ModelRefitService _service;
		private readonly ILogger<Document85_ModelRestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Document85_ModelRestService(IDocument85_ModelRefitService set_service, ILogger<Document85_ModelRestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Document85_Model_ResponseModel> FirstAsync(int id)
		{
			Document85_Model_ResponseModel result = new();
			try
			{
				ApiResponse<Document85_Model_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Document85_Model_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Document85_Model_ResponseListModel result = new();
			try
			{
				ApiResponse<Document85_Model_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Document85_Model_ResponsePaginationModel> SelectAsync(PaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Document85_Model_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
