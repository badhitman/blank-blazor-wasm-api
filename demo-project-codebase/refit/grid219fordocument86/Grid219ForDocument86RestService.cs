...
{
	/// <inheritdoc/>
	public class Grid219ForDocument86RestService : IGrid219ForDocument86RestService
	{
		private readonly IGrid219ForDocument86RefitService _service;
		private readonly ILogger<Grid219ForDocument86RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid219ForDocument86RestService(IGrid219ForDocument86RefitService set_service, ILogger<Grid219ForDocument86RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid219ForDocument86_ResponseModel> FirstAsync(int id)
		{
			Grid219ForDocument86_ResponseModel result = new();
			try
			{
				ApiResponse<Grid219ForDocument86_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid219ForDocument86_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid219ForDocument86_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid219ForDocument86_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid219ForDocument86_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid219ForDocument86_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
