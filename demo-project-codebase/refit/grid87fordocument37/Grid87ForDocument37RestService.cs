...
{
	/// <inheritdoc/>
	public class Grid87ForDocument37RestService : IGrid87ForDocument37RestService
	{
		private readonly IGrid87ForDocument37RefitService _service;
		private readonly ILogger<Grid87ForDocument37RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid87ForDocument37RestService(IGrid87ForDocument37RefitService set_service, ILogger<Grid87ForDocument37RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid87ForDocument37_ResponseModel> FirstAsync(int id)
		{
			Grid87ForDocument37_ResponseModel result = new();
			try
			{
				ApiResponse<Grid87ForDocument37_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid87ForDocument37_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid87ForDocument37_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid87ForDocument37_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid87ForDocument37_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid87ForDocument37_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
