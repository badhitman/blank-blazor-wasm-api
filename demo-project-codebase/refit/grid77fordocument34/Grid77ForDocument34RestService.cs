...
{
	/// <inheritdoc/>
	public class Grid77ForDocument34RestService : IGrid77ForDocument34RestService
	{
		private readonly IGrid77ForDocument34RefitService _service;
		private readonly ILogger<Grid77ForDocument34RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid77ForDocument34RestService(IGrid77ForDocument34RefitService set_service, ILogger<Grid77ForDocument34RestService> set_logger)
		{
		...
		{
			Grid77ForDocument34_ResponseModel result = new();
			try
			{
				ApiResponse<Grid77ForDocument34_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid77ForDocument34_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid77ForDocument34_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid77ForDocument34_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid77ForDocument34_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
