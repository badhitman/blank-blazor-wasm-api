...
{
	/// <inheritdoc/>
	public class Grid80ForDocument35RestService : IGrid80ForDocument35RestService
	{
		private readonly IGrid80ForDocument35RefitService _service;
		private readonly ILogger<Grid80ForDocument35RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid80ForDocument35RestService(IGrid80ForDocument35RefitService set_service, ILogger<Grid80ForDocument35RestService> set_logger)
		{
		...
		{
			Grid80ForDocument35_ResponseModel result = new();
			try
			{
				ApiResponse<Grid80ForDocument35_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid80ForDocument35_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid80ForDocument35_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid80ForDocument35_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid80ForDocument35_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
