...
{
	/// <inheritdoc/>
	public class Grid193ForDocument76RestService : IGrid193ForDocument76RestService
	{
		private readonly IGrid193ForDocument76RefitService _service;
		private readonly ILogger<Grid193ForDocument76RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid193ForDocument76RestService(IGrid193ForDocument76RefitService set_service, ILogger<Grid193ForDocument76RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid193ForDocument76_ResponseModel> FirstAsync(int id)
		{
			Grid193ForDocument76_ResponseModel result = new();
			try
			{
				ApiResponse<Grid193ForDocument76_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid193ForDocument76_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid193ForDocument76_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid193ForDocument76_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid193ForDocument76_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid193ForDocument76_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
