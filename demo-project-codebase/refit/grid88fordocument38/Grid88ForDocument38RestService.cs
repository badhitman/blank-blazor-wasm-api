...
{
	/// <inheritdoc/>
	public class Grid88ForDocument38RestService : IGrid88ForDocument38RestService
	{
		private readonly IGrid88ForDocument38RefitService _service;
		private readonly ILogger<Grid88ForDocument38RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid88ForDocument38RestService(IGrid88ForDocument38RefitService set_service, ILogger<Grid88ForDocument38RestService> set_logger)
		{
		...
		{
			Grid88ForDocument38_ResponseModel result = new();
			try
			{
				ApiResponse<Grid88ForDocument38_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid88ForDocument38_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid88ForDocument38_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid88ForDocument38_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid88ForDocument38_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
