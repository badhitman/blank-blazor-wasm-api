...
{
	/// <inheritdoc/>
	public class Grid58ForDocument25RestService : IGrid58ForDocument25RestService
	{
		private readonly IGrid58ForDocument25RefitService _service;
		private readonly ILogger<Grid58ForDocument25RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid58ForDocument25RestService(IGrid58ForDocument25RefitService set_service, ILogger<Grid58ForDocument25RestService> set_logger)
		{
		...
		{
			Grid58ForDocument25_ResponseModel result = new();
			try
			{
				ApiResponse<Grid58ForDocument25_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		{
			Grid58ForDocument25_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid58ForDocument25_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid58ForDocument25_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid58ForDocument25_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
