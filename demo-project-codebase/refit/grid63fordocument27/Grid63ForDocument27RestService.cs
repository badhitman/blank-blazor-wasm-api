...
{
	/// <inheritdoc/>
	public class Grid63ForDocument27RestService : IGrid63ForDocument27RestService
	{
		private readonly IGrid63ForDocument27RefitService _service;
		private readonly ILogger<Grid63ForDocument27RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid63ForDocument27RestService(IGrid63ForDocument27RefitService set_service, ILogger<Grid63ForDocument27RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid63ForDocument27_ResponseModel> FirstAsync(int id)
		{
			Grid63ForDocument27_ResponseModel result = new();
			try
			{
				ApiResponse<Grid63ForDocument27_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid63ForDocument27_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid63ForDocument27_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid63ForDocument27_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid63ForDocument27_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid63ForDocument27_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
