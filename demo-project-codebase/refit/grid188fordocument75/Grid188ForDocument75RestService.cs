...
{
	/// <inheritdoc/>
	public class Grid188ForDocument75RestService : IGrid188ForDocument75RestService
	{
		private readonly IGrid188ForDocument75RefitService _service;
		private readonly ILogger<Grid188ForDocument75RestService> _logger;

		/// <summary>
		/// Конструктор
		/// </summary>
		public Grid188ForDocument75RestService(IGrid188ForDocument75RefitService set_service, ILogger<Grid188ForDocument75RestService> set_logger)
		{
		...
		/// <inheritdoc/>
		public async Task<Grid188ForDocument75_ResponseModel> FirstAsync(int id)
		{
			Grid188ForDocument75_ResponseModel result = new();
			try
			{
				ApiResponse<Grid188ForDocument75_ResponseModel> rest = await _service.FirstAsync(id);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		/// <inheritdoc/>
		public async Task<Grid188ForDocument75_ResponseListModel> SelectAsync(IEnumerable<int> ids)
		{
			Grid188ForDocument75_ResponseListModel result = new();
			try
			{
				ApiResponse<Grid188ForDocument75_ResponseListModel> rest = await _service.SelectAsync(ids);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
		...
		public async Task<Grid188ForDocument75_ResponsePaginationModel> SelectAsync(GetByIdPaginationRequestModel request)
		{
			...
			try
			{
				ApiResponse<Grid188ForDocument75_ResponsePaginationModel> rest = await _service.SelectAsync(request);
				if (rest.StatusCode != System.Net.HttpStatusCode.OK)
				{
					result.IsSuccess = false;
					result.Message = $"HTTP error: [code={rest.StatusCode}] {rest?.Error?.Content}";
